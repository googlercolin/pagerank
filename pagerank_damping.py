...
def build_pagerank_matrix(graph: nx.DiGraph, damping_factor: float) -> np.ndarray:
    """
...
def run_pagerank_algorithm(graph: nx.DiGraph, damping_factor: float, max_iterations: int) -> np.ndarray:
    """
    ...
    pagerank_matrix = build_pagerank_matrix(graph, damping_factor)
    ...
    plt.savefig(f'convergence_plots_damping/{damping_factor}.png')  # Save the plot as PNG
    plt.close(fig)  # Close the plot to prevent it from being displayed

    return final_state

def plot_graph(graph: nx.DiGraph, final_probs: np.ndarray, node_int_dict: dict, bool_final_probs: bool = False, node_size: int = 500, damping_factor = 0.85, ax=None):
    ...
    if bool_final_probs:
        ...
        plt.savefig(f'graphs_damping/graph_with_pagerank_{damping_factor}.png')  # Save the plot as PNG
        plt.close()  # Close the plot to prevent it from being displayed
    else:
        nx.draw(graph, with_labels=False, node_size=node_size, ax=ax, pos=pos)
        nx.draw_networkx_labels(graph, pos, labels=labels, font_size=node_size/40, font_color='maroon', ax=ax)
        plt.savefig('graphs_damping/graph.png')  # Save the plot as PNG
        plt.close()  # Close the plot to prevent it from being displayed

def compute_and_plot_pagerank(graph: nx.Graph, damping_factor: float, max_iterations: int) -> np.ndarray:
    """
    ...
    final_probs = run_pagerank_algorithm(directed_graph, damping_factor, max_iterations)
    ...
    plot_graph(directed_graph, final_probs, node_int_dict, bool_final_probs=True, damping_factor=damping_factor)
...
if __name__ == "__main__":
    ...
    damping_factors = [0.8, 0.85, 0.9, 0.95, 0.99]  # List of damping factors to experiment with
    
    for damping_factor in damping_factors:
        final_probs_power_iteration = compute_and_plot_pagerank(graph, damping_factor, max_iterations)

        # Calculate the Gini coefficient
        gini_coefficient = gini(final_probs_power_iteration)
        
        print(f"\nPageRank Scores (Power iteration) for damping factor {damping_factor}:")
